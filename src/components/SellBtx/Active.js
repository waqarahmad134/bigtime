...
export default function Active() {
  ...
  useEffect(() => {
    const fetchActiveListings = async () => {
      try {
        ...
        // Only keep listings that are active
        const activeListings = (res.results || []).filter(
  ...
  if (listings.length === 0)
    return <p className="text-gray-300 text-center mt-10">No active listings found.</p>;
